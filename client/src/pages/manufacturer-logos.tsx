...
export default function ManufacturerLogos() {
  ...
  // Logo mapping for automatic linking
  ...
  const linkAllLogos = async () => {
    ...
    for (const manufacturer of manufacturers) {
      ...
      if (logoPath && !manufacturer.logo) {
        try {
          const response = await fetch(`/api/manufacturers/${manufacturer.id}/logo`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (response.ok) {
            ...
            console.log(`❌ Failed to update logo for: ${manufacturer.nameAr}`);
          }
        } catch (error) {
    ...
    toast({
      title: "تم ربط الشعارات",
      description: `تم ربط ${updatedCount} شعار، تم تخطي ${skippedCount} شركة`,
    });
  };
