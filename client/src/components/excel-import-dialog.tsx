      ...
      try {
        ...
        const mappedData = dataRows.map((row: any[], index: number) => {
          return {
            ...
            fuelType: row[5] || 'بنزين',
            transmission: row[6] || 'أوتوماتيك',
            drivetrain: row[7] || 'دفع خلفي',
            exteriorColor: row[8] || '',
            interiorColor: row[9] || '',
            mileage: row[10] ? parseInt(row[10]) : 0,
            price: row[11] ? parseFloat(row[11]) : 0,
            location: row[12] || 'الرياض',
            status: row[13] || 'متاح للبيع',
            importType: row[14] || 'وكالة',
            ownershipType: row[15] || 'ملكية شخصية',
            notes: row[16] || '',
            chassisNumber: row[17] || `CH${Date.now()}${index}`,
            serialNumber: row[18] || `SN${Date.now()}${index}`,
          };
        });
