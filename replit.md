...
- **Design Principles**: Clean minimal black & white theme inspired by reference design. Light gray page background (#EBEBEB) with pure white cards/panels floating on top. Pure black (#000000) as the only accent color. Dark sidebar (#1A1A1A) and dark dropdown menus (#1A1A1A with white text). No CSS transitions globally (`transition: none !important`). RTL Arabic support throughout.
- **Color System**: `--background: hsl(0,0%,92%)` (#EBEBEB page bg), `--card: hsl(0,0%,100%)` (#FFFFFF white cards), `--primary: hsl(0,0%,0%)` (#000000 black), `--border: hsl(0,0%,88%)` (#E0E0E0 subtle gray), `--popover: hsl(0,0%,10%)` (#1A1A1A dark dropdowns). Login page uses gray background, sidebar stays dark.
