...
QR Code Scanning: Added comprehensive QR code scanning functionality with camera access for scanning vehicle QR codes. When a QR code is scanned, the system opens a detailed vehicle dialog showing the full vehicle information including images, specifications, status, and price. The dialog includes three main action buttons (حجز/Reserve, بيع/Sell, مشاركة/Share) exactly as requested. QR scanner button is integrated into the main header navigation next to attendance management button for easy access.
